...
exports.handler = async (event) => {
  ...
  const { firstName, email, propertyName, propertyAddress, channel, unitCount } = body;
  if (!email) {
    return { statusCode: 400, body: JSON.stringify({ error: 'Missing email' }) };
  }

  const channelLabel = channel === 'email' ? 'Email' : channel === 'sms' ? 'SMS' : 'Email & SMS';
  const unitLabel = unitCount ? `~${unitCount} units` : 'Unknown';
      ...
      <td style="background:#1e3448;border-radius:16px 16px 0 0;padding:36px 36px 32px;">
        ...
        <table width="100%" cellpadding="0" cellspacing="0" border="0" style="background:#152a3a;border-radius:10px;border:1px solid #2a4a64;">
          <tr><td style="padding:20px 24px;">
            <p style="margin:0 0 14px;font-size:11px;font-weight:700;color:#00B2A9;letter-spacing:0.07em;text-transform:uppercase;">Property submitted</p>
            <table cellpadding="0" cellspacing="0" border="0" width="100%">
              ...
              <tr>
                ...
                <td width="50%">
                  <p style="margin:0 0 2px;font-size:10px;color:#5a7a90;text-transform:uppercase;letter-spacing:0.06em;">Est. residents</p>
